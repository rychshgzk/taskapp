...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    var taskArray = try!
        Realm().objects(Task.self).sorted(byKeyPath: "date", ascending: false)
    ...
    // UISearchBarプロトコルのメソッド
    // 検索バーにテキスト入力完了した時に呼び出す
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        taskArray = realm.objects(Task.self).filter("category BEGINSWITH %@",searchCategory.text!).sorted(byKeyPath: "date", ascending: false)
        tableView.reloadData()
    }
